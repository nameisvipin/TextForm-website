...
export default function TextForm(prop) {
  ...
  const handleEmail = () => {
    ...
    if (emails) {
      let newMail = emails.join(" ");
      setMail(newMail);
      prop.showAlert("Emails are copied to clipboard", "success");
    } else {
      setMail("");
      prop.showAlert("No emails found", "warning");
    }
  };
  ...
  let readingTime = Math.ceil(wordCount / 200); // Assuming an average reading speed of 200 words per minute
  ...
  return (
    <>
      ...
      <div className="container my-3">
        ...
        <p>{readingTime} minute(s) read</p>
        <h2>Preview</h2>
        <p>{text.length > 0 ? text : 'Enter something in the text area above to preview it here...'}</p>
        <h2>The email(s) found in the given text is/are: </h2>
        <p>{mail}</p>
      </div>
    </>
  );
